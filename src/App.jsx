...
const HOR_FDS = [
  { label:"08h às 17h", colKey:"fds_0817", col:1 },
  { label:"10h às 20h", colKey:"fds_1020", col:2 },
  { label:"12h às 22h", colKey:"fds_1222", col:3 },
];
// Horário padrão (Seg–Sex)
const HOR_SEG = [{ label:"09h às 18h", colKey:"semana_0918", col:4 }];
// Horários casamento/debutante (Sex/Sáb/Dom/Fer)
const HOR_FDS_ESP = [
  { label:"08h às 17h", colKey:"fds_0817", col:1 },
  { label:"09h às 02h", colKey:"fds_0902", col:2 },
];
// Horário casamento/debutante (Seg–Qui)
const HOR_SEG_ESP = [{ label:"09h às 00h", colKey:"semana_0900", col:3 }];
...
function lookupPreco(tabela, nPessoas, colKey) {
  if (!tabela||!tabela.length||!colKey) return 0;
  const row = tabela.find(r=>r.pessoas>=nPessoas)||tabela[tabela.length-1];
  return row ? (row[colKey]||0) : 0;
}
...
export default function App() {
  ...
  const getTipoDiaCtx = ds => {
    ...
    if(isEspecial && td==="semana" && fromStr(ds).getDay()===5) return "fds";
    return td;
  };
  ...
  const getHorList = ds => {
    ...
    const td=getTipoDia(ds);
    return (td==="fds"||td==="feriado") ? HOR_FDS : HOR_SEG;
  };
  ...
  const getTabela = () => {
    if(precosCfg){
      if(isEspecial && precosCfg.eventos_especiais?.[eventoAtual.tabela]?.length)
        return precosCfg.eventos_especiais[eventoAtual.tabela];
      if(precosCfg.tabela_padrao?.length) return precosCfg.tabela_padrao;
    }
    return isEspecial ? TABELA_ESPECIAL : TABELA_PADRAO;
  };

  const calcDia = ds => {
    if(getTipoDiaCtx(ds)==="especial") return 0;
    const h=getHor(ds), tab=getTabela();
    return lookupPreco(tab,nPessoas,h.colKey);
  };
  ...
  const taxaLimp    = isEspecial ? (precosCfg?.taxa_limpeza_especial??300) : (precosCfg?.taxa_limpeza??TAXA_LIMPEZA);
    ...
    <div style={{fontFamily:"Segoe UI,Arial,sans-serif",minHeight:"100vh",background:"#faf8f8",display:"flex",flexDirection:"column"}}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        .site-header{background:#ffffff;color:white;box-shadow:0 2px 12px rgba(0,0,0,.2)}
        ...
        .header-info-item{display:flex;align-items:center;gap:8px;font-size:13px;opacity:.9}
      ...
      <main className="main-content">
        <div style={{display:"flex",gap:12,flexWrap:"wrap",marginBottom:"0.75rem",fontSize:12,color:"#555"}}>
          {[["#639922","Disponível"],["#a5a5a5","Reservado"]].map(([c,l])=>(
          ...
          {!loading && (
            <>
              ...
              <div style={{display:"flex",gap:16,marginTop:"1rem",flexWrap:"wrap"}}>
                {[["#639922","Disponível"],["#a5a5a5","Reservado"],["#823122","Selecionado"]].map(([c,l])=>(
        ...
        <div className="footer-inner">
          <div className="footer-grid">
            ...
            <div className="footer-col">
              <h4>Contato</h4>
              <ul>
                <li><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 014.49 12 19.79 19.79 0 011.42 3.38 2 2 0 013.4 1.21h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L7.09 9a16 16 0 006.29 6.29l1.86-1.65a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z"/></svg>(11) 9 95384-0934</li>
                <li><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>chacaradossonhosembu@gmail.com
</li>
                ...
                <li>
                  ...
                  <a href="https://instagram.com/chacara_dos_sonhos" target="_blank" rel="noreferrer" style={{color:"rgba(255,255,255,0.75)",textDecoration:"none"}}>@chacara_dos_sonhos</a>
                </li>
              </ul>
            </div>
            <div className="footer-col">
